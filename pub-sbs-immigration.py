...
def get_news(host, database, user, password):
    try:
        ...
        if connection.is_connected():
            ...
            #cursor.execute("select id, title, linkUrl from SBS_australian_immigration_items where published is null and (type = 'article' or type = 'podcast-episode') AND title NOT LIKE '%SBS%' order by create_time DESC LIMIT 1;")
            cursor.execute(
                "select id, title, linkUrl from SBS_australian_immigration_items where published is null and type = 'article' AND title NOT LIKE '%SBS%' order by create_time DESC LIMIT 1;")
            ...
            if result:
                ...
                sql = "UPDATE `SBS_australian_immigration_items` SET published = '1' WHERE id = %s;"
...
#get_sbs_content('https://www.sbs.com.au/language/chinese/zh-hans/podcast-episode/selling-art-after-just-one-year-of-learning-all-you-need-is-passion-and-insight/u6wbmz6j9')
