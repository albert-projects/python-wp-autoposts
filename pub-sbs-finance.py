...
def wordpress_post(title, feature_img, post_body):
    ...
    #user = 'oz1'
    #password = 'jsWG eeim Fmqe NtD6 nmXQ p9ww'
    ...
    # url_polylang = f"https://ozeasy.com/wp-json/custom/v1/update-post/{postID}"
    # data_polylang = {
    ...
    #tmp_php = f"<?php pll_save_post_translations(array('zh'=>{postID}));  ?>"
    #post_body_cn.append(tmp_php)
...
def get_news(host, database, user, password):
    try:
        ...
        if connection.is_connected():
            ...
            #cursor.execute("select id, title, linkUrl from SBS_australian_finance_items where published is null and (type = 'article' or type = 'podcast-episode') AND title NOT LIKE '%SBS%' order by create_time DESC LIMIT 1;")
            cursor.execute(
                "select id, title, linkUrl from SBS_australian_finance_items where published is null and type = 'article' AND title NOT LIKE '%SBS%' order by create_time DESC LIMIT 1;")
            ...
            if result:
                ...
                sql = "UPDATE `SBS_australian_finance_items` SET published = '1' WHERE id = %s;"
...
#get_sbs_content('https://www.sbs.com.au/language/chinese/zh-hans/article/key-takeaways-from-the-migration-review/d9lcvuoh6')
