...
def sbs_AuNews():
    ...
    url = 'https://www.sbs.com.au/language/chinese/zh-hans/collection/business-finance'
...
def insert_content(host, database, user, password):
    try:
        ...
        if connection.is_connected():
            ...
            '''
            # insert html_content
            sql = "INSERT INTO `SBS_australian_finance` (`html_content`) VALUES (%s);"
            new_data = content
            cursor = connection.cursor()
            ...
            cursor.execute("SELECT `id`, `html_content` FROM `SBS_australian_finance` ORDER BY `page_time` DESC LIMIT 1;")
            ...
            for row in records:
                ...
                for r in content_json:
                    ...
                    cursor.execute("SELECT id from `SBS_australian_finance_items` where id='" + id + "'")
                    rows_count = cursor.fetchone()
                    if rows_count == None:
                         # insert into another table
                         sql = """INSERT INTO `SBS_australian_finance_items` (ref_id, id, type, title, image, linkUrl, permalink)
