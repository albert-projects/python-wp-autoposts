...
def get_news(host, database, user, password):
    try:
        ...
        if connection.is_connected():
            ...
            #cursor.execute("select id, title, linkUrl from SBS_australian_living_items where published is null and (type = 'article' or type = 'podcast-episode') AND title NOT LIKE '%SBS%' order by create_time DESC LIMIT 1;")
            cursor.execute(
                "select id, title, linkUrl from SBS_australian_living_items where published is null and type = 'article' AND title NOT LIKE '%SBS%' order by create_time DESC LIMIT 1;")
            ...
            if result:
                ...
                sql = "UPDATE `SBS_australian_living_items` SET published = '1' WHERE id = %s;"
...
#get_sbs_content('https://www.sbs.com.au/language/chinese/zh-hans/podcast-episode/sbs-news-flash-greens-call-for-new-housing-rent-bill/xvge9gh3k')
