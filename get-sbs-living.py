...
def sbs_AuNews():
    ...
    url_list = ['https://www.sbs.com.au/language/chinese/zh-hans/collection/cost-of-living',
           'https://www.sbs.com.au/language/chinese/zh-hans/collection/health',
           'https://www.sbs.com.au/language/chinese/zh-hans/collection/education',
           'https://www.sbs.com.au/language/chinese/zh-hans/collection/food-travel-entertainment',
           'https://www.sbs.com.au/language/chinese/zh-hans/collection/cultural-events'
           ]
    tmp_content = []
    ...
    for url in url_list:
        ...
        tmp_content.append(items_list)
        #print(items_list)

    # Convert to string
    content = str(tmp_content)
...
def insert_content(host, database, user, password):
    try:
        ...
        if connection.is_connected():
            ...
            '''
            # insert html_content
            sql = "INSERT INTO `SBS_australian_living` (`html_content`) VALUES (%s);"
            new_data = content
            cursor = connection.cursor()
            ...
            cursor.execute("SELECT `id`, `html_content` FROM `SBS_australian_living` ORDER BY `page_time` DESC LIMIT 1;")
            ...
            for row in records:
                ...
                for r in content_json:
                    ...
                    cursor.execute("SELECT id from `SBS_australian_living_items` where id='" + id + "'")
                    rows_count = cursor.fetchone()
                    if rows_count == None:
                         # insert into another table
                         sql = """INSERT INTO `SBS_australian_living_items` (ref_id, id, type, title, image, linkUrl, permalink)
